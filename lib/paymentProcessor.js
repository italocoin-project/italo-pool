    ...
    async.waterfall([
        ...
        function(balances, minPayoutLevel, callback){
            ...
            for (var worker in balances){
                ...
                {
                    if (config.payments.minPaymentExchange)
                    {
                        //Check a second time to make sure balance is above threshold for exchange wallet
                        var address = worker;
                        ...
                        {
                            if (balance >= config.payments.minPaymentExchange)
                            {
                    ...
                    {
                        //Otherwise, process payment as normal
                        var remainder = balance % config.payments.denomination;
                        var payout = balance - remainder;
            ...
            for (var worker in payments){
                ...
                if ((addr.length === 1 && utils.isIntegratedAddress(address)) || addr.length >= 2){
                    ...
                    if (addr.length >= 2){
                        ...
                        else if (payment_id.length == 16)
                        {
                            payment_id = payment_id + "000000000000000000000000000000000000000000000000";
                        }
                    }
                    if (addresses > 0){
                        commandIndex++;
