...
var cuHashing = require('cuckaroo29i-hashing');
...
var cnAlgorithm = config.cnAlgorithm || "cuckaroo29i";
var cnVariant = config.cnVariant || 0;
var cnBlobType = config.cnBlobType || 12;
...
function recordShareData(miner, job, shareDiff, blockCandidate, hashHex, shareType, blockTemplate){
    ...
    var redisCommands = [
        ...
        ['zadd', config.coin + ':hashrate', dateNowSeconds, [job.difficulty*48, miner.login, dateNow].join(':')],
        ['hincrby', config.coin + ':workers:' + miner.login, 'hashes', job.difficulty*48],
    ...
    if (miner.workerName) {
        redisCommands.push(['zadd', config.coin + ':hashrate', dateNowSeconds, [job.difficulty*48, miner.login + '~' + miner.workerName, dateNow].join(':')]);
        redisCommands.push(['hincrby', config.coin + ':unique_workers:' + miner.login + '~' + miner.workerName, 'hashes', job.difficulty*48]);
...
function processShare(miner, job, blockTemplate, params){
    ...
    prooferror = cuHashing.cuckaroo29i(header,params.pow);
