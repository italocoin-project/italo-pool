...
var cryptoNight;
if(config.cnAlgorithm !== "cuckaroo29s")
{
...
function processBlockTemplate(template){
    ...
    for (var minerId in connectedMiners){
        var miner = connectedMiners[minerId];
        miner.pushMessage((miner.typeB)?'getjobtemplate':'job', miner.getJob(),miner.typeB, minerId);
        if(miner.typeB) miner.pushMessage('message', {'message':'new block at height: '+currentBlockTemplate.height},miner.typeB, minerId);
    }
}
...
Miner.prototype = {
    ...
    setNewDiff: function(newDiff){
        ...
        this.pushMessage((this.typeB)?'getjobtemplate':'job', this.getJob(),this.typeB,this.id);
        if(this.typeB) this.pushMessage('message', {'message':'retarget diff to: '+newDiff},this.typeB, this.id);
    },
    heartbeat: function(){
...
function handleMinerMethod(method, params, ip, portData, sendReply, pushMessage, setMinerid){
    ...
    if((!miner)&&(method==='submit')&&(params.job_id)) miner = connectedMiners[params.job_id];
    ...
    switch(method){
        case 'login':
            ...
            if(miner.typeB) {
                miner.pushMessage('login', 'ok',miner.typeB,minerId);
        ...
        case 'submit':
            ...
            if (!job){
                if(!miner.typeB) sendReply('Invalid job id');
                if(miner.typeB)  sendReply({code: -32503, message: "Invalid job id"}, null, 'submit');
                return;
            }


            if (!params.nonce || (!params.result && !miner.typeB)) {
            ...
            if (!miner.typeB && !noncePattern.test(params.nonce)) {
                var minerText = miner ? (' ' + miner.login + '@' + miner.ip) : '';
            ...
            if(!miner.typeB) params.nonce = params.nonce.toLowerCase();
            if(miner.typeB) params.nonce = params.nonce.toString().toLowerCase();
        ...
        if (!miner.proxy) {
            if (job.submissions.indexOf(params.nonce) !== -1){
                ...
                perIPStats[miner.ip] = { validShares: 0, invalidShares: 999999 };
                miner.checkBan(false);
                if(!miner.typeB) sendReply('Duplicate share');
                if(miner.typeB) sendReply({code: -32505, message: "Duplicate share"}, null, 'submit');
                return;
            }
            ...
            if (!blockTemplate){
                if(!miner.typeB) sendReply('Block expired');
                if(miner.typeB) sendReply({code: -32600, message: "Block expired"}, null, 'submit');
                return;
            }
            ...
            if (!shareAccepted){
                if(!miner.typeB) sendReply('Rejected share: invalid result');
                if(miner.typeB)  sendReply({code: -32502, message: "wrong hash"}, null, 'submit');
                return;
            }
            ...
            if(!miner.typeB) sendReply(null, {status: 'OK'});
            if(miner.typeB)  sendReply(null, 'ok', 'submit');
            break;
        case 'keepalived' :
            if (!miner){
                sendReply('Unauthenticated');
...
function startPoolServerTcp(callback){
    ...
    async.each(config.poolServer.ports, function(portData, cback){
        var handleMessage = function(socket, jsonData, pushMessage){
            ...
            if( jsonData.result ) {
                handleMinerMethod(jsonData.method, jsonData.result, socket.remoteAddress, portData, sendReply, pushMessage);
            }
            else{
        ...
        var socketResponder = function(socket){
            ...
            var pushMessage = function(method, params,typeB,id){
                if(!socket.writable) return;
                if(typeB)
                {
                    var sendData = JSON.stringify({
